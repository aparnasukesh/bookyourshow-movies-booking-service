...
func (h *GrpcHandler) ListShowTimeByTheaterID(ctx context.Context, req *movie_booking.ListShowTimeByTheaterIdRequest) (*movie_booking.ListShowTimeByTheaterIdResponse, error) {
	showtimes, theater, err := h.svc.ListShowTimeByTheaterID(ctx, int(req.TheaterId))
	if err != nil {
		return nil, err
	}
	theaterRes := movie_booking.Theater{
	...
	return &movie_booking.ListShowTimeByTheaterIdResponse{
		Theater:  &theaterRes,
		ShowTime: showtimeRes,
	}, nil
...
func (h *GrpcHandler) GetScreensAndMovieScedulesByTheaterID(ctx context.Context, req *movie_booking.GetScreensAndMovieScedulesByTheaterIdRequest) (*movie_booking.GetScreensAndMovieScedulesByTheaterIdResponse, error) {
	screens, movieSchedule, theater, err := h.svc.GetScreensAndMovieSchedulesByTheaterID(ctx, int(req.TheaterId))
	if err != nil {
		return nil, err
	}
	theaterRes := &movie_booking.Theater{
	...
	movieScheduleRes := []*movie_booking.MovieSchedule{}
	for _, schedule := range movieSchedule {
		res := movie_booking.MovieSchedule{
			...
			Movie: &movie_booking.Movie{
				MovieId:     uint32(schedule.MovieID),
				Title:       schedule.Movie.Title,
				Description: schedule.Movie.Description,
				Duration:    int32(schedule.Movie.Duration),
				Genre:       schedule.Movie.Genre,
				ReleaseDate: schedule.Movie.ReleaseDate.String(),
				Rating:      float32(schedule.Movie.Rating),
				Language:    schedule.Movie.Language,
			},
			Theater: theaterRes,
			ShowTime: &movie_booking.Showtime{
				Id:       uint32(schedule.ShowtimeID),
				MovieId:  int32(schedule.Showtime.MovieID),
				ScreenId: int32(schedule.Showtime.ScreenID),
				ShowDate: timestamppb.New(schedule.Showtime.ShowDate),
				ShowTime: timestamppb.New(schedule.Showtime.ShowDate),
				Movie:    &movie_booking.Movie{},
				TheaterScreen: &movie_booking.TheaterScreen{
					ID:           uint32(schedule.Showtime.ScreenID),
					TheaterID:    int32(schedule.Showtime.TheaterScreen.TheaterID),
					ScreenNumber: int32(schedule.Showtime.TheaterScreen.ScreenNumber),
					SeatCapacity: int32(schedule.Showtime.TheaterScreen.SeatCapacity),
					ScreenTypeID: int32(schedule.Showtime.TheaterScreen.ScreenTypeID),
					Theater:      theaterRes,
					ScreenType:   &movie_booking.ScreenType{},
				},
			},
		}
		movieScheduleRes = append(movieScheduleRes, &res)
	}

	theaterScreenRes := []*movie_booking.TheaterScreen{}

	for _, screen := range screens {
		res := movie_booking.TheaterScreen{
			ID:           uint32(screen.ID),
			TheaterID:    int32(screen.TheaterID),
			ScreenNumber: int32(screen.ScreenNumber),
			SeatCapacity: int32(screen.SeatCapacity),
			ScreenTypeID: int32(screen.ScreenTypeID),
			Theater:      theaterRes,
			ScreenType: &movie_booking.ScreenType{
				Id:             int32(screen.ScreenType.ID),
				ScreenTypeName: screen.ScreenType.ScreenTypeName,
			},
		}
		theaterScreenRes = append(theaterScreenRes, &res)
	}

	return &movie_booking.GetScreensAndMovieScedulesByTheaterIdResponse{
		Theater:       theaterRes,
		MovieSchedule: movieScheduleRes,
		TheaterScreen: theaterScreenRes,
	}, nil
...
func (h *GrpcHandler) GetSeatsByScreenID(ctx context.Context, req *movie_booking.GetSeatsByScreenIDRequest) (*movie_booking.GetSeatsByScreenIDResponse, error) {
	...
	for _, seat := range seats {
		grpcSeat := &movie_booking.Seat{
			...
			TheaterScreen: &movie_booking.TheaterScreen{
				ID:           uint32(seat.TheaterScreen.ID),
				TheaterID:    int32(seat.TheaterScreen.TheaterID),
				ScreenNumber: int32(seat.TheaterScreen.ScreenNumber),
				SeatCapacity: int32(seat.TheaterScreen.SeatCapacity),
				ScreenTypeID: int32(seat.TheaterScreen.ScreenTypeID),
				Theater:      &movie_booking.Theater{},
				ScreenType:   &movie_booking.ScreenType{},
			},
			SeatCategory: &movie_booking.SeatCategory{
				Id:               int32(seat.SeatCategory.ID),
				SeatCategoryName: seat.SeatCategory.SeatCategoryName,
			},
		}
...
func (h *GrpcHandler) GetAvailableSeatsByScreenIdAndShowTimeID(ctx context.Context, req *movie_booking.GetAvailableSeatsByScreenIDAndShowTimeIDRequest) (*movie_booking.GetAvailableSeatsByScreenIDAndShowTimeIDResponse, error) {
	...
	return &movie_booking.GetAvailableSeatsByScreenIDAndShowTimeIDResponse{Seats: grpcSeats}, nil
}
...
func (h *GrpcHandler) ListShowtimesByShowDateAndMovieID(ctx context.Context, req *movie_booking.ListShowtimesByShowDateAndMovieIdRequest) (*movie_booking.ListShowtimesByShowDateAndMovieIdResponse, error) {
	showtimes, err := h.svc.ListShowtimesByShowDateAndMovieID(ctx, req.ShowDate.AsTime(), int(req.MovieId))
	if err != nil {
		return nil, err
	}
	response := []*movie_booking.Showtime{}
	for _, showtime := range showtimes {
		res := movie_booking.Showtime{
			Id:       uint32(showtime.ID),
			MovieId:  int32(showtime.MovieID),
			...
			TheaterScreen: &movie_booking.TheaterScreen{
				...
				ScreenTypeID: int32(showtime.TheaterScreen.ScreenTypeID),
				Theater: &movie_booking.Theater{
					TheaterId:       int32(showtime.TheaterScreen.Theater.ID),
					Name:            showtime.TheaterScreen.Theater.Name,
					Place:           showtime.TheaterScreen.Theater.Place,
					City:            showtime.TheaterScreen.Theater.City,
					District:        showtime.TheaterScreen.Theater.District,
					State:           showtime.TheaterScreen.Theater.State,
					OwnerId:         uint32(showtime.TheaterScreen.Theater.OwnerID),
					NumberOfScreens: int32(showtime.TheaterScreen.Theater.NumberOfScreens),
					TheaterTypeId:   int32(showtime.TheaterScreen.Theater.TheaterTypeID),
					TheaterType: &movie_booking.TheaterType{
						Id:              int32(showtime.TheaterScreen.Theater.TheaterType.ID),
						TheaterTypeName: showtime.TheaterScreen.Theater.TheaterType.TheaterTypeName,
					},
				},
				ScreenType: &movie_booking.ScreenType{},
			},
	...
	return &movie_booking.ListShowtimesByShowDateAndMovieIdResponse{
		Showtimes: response,
	}, nil
}
